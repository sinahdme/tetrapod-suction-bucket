...
%% Liu et al. (2024), Prototype Scale (1:100)
do_b = 15.0; %bucket outer diameter [m] (Liu Table 1: 150mm x 100)
thickness = 0.100; %bucket wall thickness [m] (Liu Table 1: 1mm x 100)
l_b = 15.0; %length of the bucket skirt [m] (Liu Table 1: 150mm x 100)
Z_lid =0.001; % thickness of the lid of bucket
n = 16; %number of strips in a single circumference
n_l =18; % number of layers along skirt
n_q =10; % radial rings for lid
eccentricity_base = 25.0; % tower CoG height [m] (base value, adjusted per depth)
th1= 45;    % bucket 1 position [deg] (NE)
th2 = 135;  % bucket 2 position [deg] (NW)
th3 = 225;  % bucket 3 position [deg] (SW)
th4 = 315;  % bucket 4 position [deg] (SE)
Deadload=10000; %kN (Liu: vertical dead load)
S = 15.0; % center-to-center spacing [m] (Liu Table 1)
half_S = S / 2;
l = sqrt(2) * half_S; % circumradius [m] = sqrt(2)*7.5 = 10.607 m
%% %%%%%%%%%% Soil parameters %%%%%%%%%%%%
gamma = 9.5; % buoyant unit weight [kN/m3] (Liu/Python: gamma_sub=9.5)
S_gamma = 0.6; % shape factor (qzcurve4)
e_max =0;
e_min =0;
D_r = 0.70; % relative density (Liu: Dr=0.70)
...
fi_crit = 33; % critical state friction angle [deg] (Liu: phi_cv=33.0)
m =3; % for finding the FI_peak %m is either 0 or 3
delta = 0.7 * fi_crit; % interface friction [deg] (Liu: delta_ratio=0.7)
%%
...
penetration_depths = [4, 5, 7, 9, 11, 13, 15];  % penetration depths to analyze [m] — 8 values
...
% Exploit 4-fold symmetry: only compute 0 to <90 deg, then replicate
symmetry_order = 4;  % tetrapod has 4-fold symmetry
sector_angle = 360 / symmetry_order;  % 90 degrees per sector
tilt_directions_compute = 45:15:(45 + sector_angle - 15);  % 45, 60, ..., 120 (one sector starting from bucket 1)
...
dd_steps = deg2rad([0.5, 1.0, 1.5, 2.0, 2.5, 3.0]);  % [rad] — 6 values
...
penetration4 = penetration_depths(i_pen);
% Depth-dependent eccentricity: exposed skirt above mudline adds to moment arm
eccentricity = eccentricity_base + (l_b - penetration_depths(i_pen));
fprintf('\n\n############################################################\n');
...
fprintf('\n  >> dir=%d deg, dd=%.1f deg => MOMENT=%.2f, F_HH=%.2f, ecc=%.1f m\n', tilt_dir, dd*180/pi, MOMENT, F_HH, eccentricity);
...
% Map computed sector (45 to <135 deg) to full 360 deg
...
%% $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  SAVE RESULTS  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ %%
fprintf('\n===== Saving results to FOUR_POD_POLAR_results_ver03.mat =====\n');
save('FOUR_POD_POLAR_results_ver03.mat', 'M_polar', 'F_H_polar', 'Q_store', ...
    ...
    'fi_crit', 'delta', 'Deadload', 'thickness', 'eccentricity_base');
fprintf('===== M_polar shape: (%d, %d, %d) = (n_alpha, n_theta, n_depth) =====\n', size(M_polar,1), size(M_polar,2), size(M_polar,3));

%% $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  WRITE CSV LUT  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ %%
fprintf('\n===== Writing FOUR_POD_POLAR_LUT_ver03.csv =====\n');
fid = fopen('FOUR_POD_POLAR_LUT_ver03.csv', 'w');
fprintf(fid, 'alpha_deg,theta_deg,depth_m,M_kNm,F_H_kN\n');
for i_pen = 1:num_pen
    for i_step = 1:num_steps
        for i_dir = 1:num_dirs
            fprintf(fid, '%d,%.1f,%d,%.2f,%.2f\n', ...
                tilt_directions(i_dir), dd_steps(i_step)*180/pi, ...
                penetration_depths(i_pen), M_polar(i_dir, i_step, i_pen), ...
                F_H_polar(i_dir, i_step, i_pen));
        end
    end
end
fclose(fid);
fprintf('===== CSV written: %d rows =====\n', num_dirs * num_steps * num_pen);
...
for i_pen = 1:num_pen
    ...
    ecc_val = eccentricity_base + (l_b - penetration_depths(i_pen));
    ...
    hold on;
    legend_labels1{i_pen} = sprintf('L_p = %d m (L/D = %.2f, e = %.0f m)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b, ecc_val);
end
title(sprintf('Polar Moment at %.1f deg Rotation (Four Pod, Liu 2024)', dd_steps(end)*180/pi), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
...
for i_pen = 1:num_pen
    ...
    polarplot(tilt_dirs_rad_closed, F_closed/1000, line_styles{i_pen}, 'LineWidth', 2.0, ...
        'Color', colors_pen(i_pen,:), 'MarkerSize', 5, 'MarkerFaceColor', colors_pen(i_pen,:));
    hold on;
    legend_labels2{i_pen} = sprintf('L_p = %d m (L/D = %.2f, e = %.0f m)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b, ecc_val);
end
title(sprintf('Polar Horizontal Force at %.1f deg Rotation (Four Pod, Liu 2024)', dd_steps(end)*180/pi), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
...
for i_pen = 1:num_pen
    ...
    ecc_val = eccentricity_base + (l_b - penetration_depths(i_pen));
    title(sprintf('L_p = %d m (e=%.0f)', penetration_depths(i_pen), ecc_val), 'FontSize', 12, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
    set(gca, 'fontweight', 'bold', 'fontsize', 10);
    ax = gca;
    ax.ThetaDir = 'counterclockwise';
    ax.ThetaZeroLocation = 'right';
end
sgtitle('Polar Moment-Rotation Diagram (Four Pod, Liu 2024)', 'FontSize', 18, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
...
for i_pen = 1:num_pen
    ...
    hold on;
    ecc_val = eccentricity_base + (l_b - penetration_depths(i_pen));
    legend_labels4{i_pen} = sprintf('L_p = %d m (L/D = %.2f, e = %.0f m)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b, ecc_val);
end
grid on;
...
title(sprintf('Moment vs Rotation at %d deg Tilt Direction (Four Pod, Liu 2024)', tilt_directions_compute(1)), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
...
for i_pen = 1:num_pen
    ...
    ecc_val = eccentricity_base + (l_b - penetration_depths(i_pen));
    title(sprintf('L_p = %d m (e=%.0f m)', penetration_depths(i_pen), ecc_val), 'FontSize', 12, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
end
sgtitle(sprintf('Moment vs Rotation (%d deg-%d deg sector, Four Pod, Liu 2024)', tilt_directions_compute(1), tilt_directions_compute(end)), 'FontSize', 18, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
% Add shared legend
legend_labels5 = {};
for i_dir = 1:num_dirs_compute
...
%% Figure 6: Eccentricity vs Penetration Depth
figure(6);
ecc_values = eccentricity_base + (l_b - penetration_depths);
plot(penetration_depths, ecc_values, '-ko', 'LineWidth', 2.5, 'MarkerSize', 8, 'MarkerFaceColor', 'r');
grid on;
set(gca,'fontweight','bold','fontsize',18);
xlabel('Penetration Depth (m)', 'FontSize', 22, 'FontWeight', 'bold','FontName','Times New Roman');
ylabel('Effective Eccentricity (m)', 'FontSize', 22, 'FontWeight', 'bold','FontName','Times New Roman');
title('Eccentricity = h_{cog} + (L_b - L_p)', 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
text(penetration_depths(end), ecc_values(end)+0.5, sprintf('Fully penetrated: e = %.0f m', eccentricity_base), 'FontSize', 14, 'FontWeight', 'bold');
