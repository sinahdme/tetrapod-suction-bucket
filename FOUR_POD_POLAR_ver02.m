...
do_b = 6.5; %bucket outer diameter (Barari: D=6.5m)
thickness = 0.025; %bucket thickness (Barari: 25mm)
l_b = 6.5; %length of the bucket skirt (Barari: L/D=1.0)
...
n_q =10;
eccentricity = 33; % load eccentricity (Barari: 33m)
th1= 45;
th2 = 135;
th3 = 225;
th4 = 315;
Deadload=5000; %kN (Barari: 5 MN)
l = 19.5; % circumradius (Barari: S/D=3.0, S=19.5m)
%% %%%%%%%%%% Soil parameters %%%%%%%%%%%%
gamma = 9.21; % buoyant unit weight [kN/m3] - Barari: Saemangeum silty sand
S_gamma = 0.5;
e_max =0;
e_min =0;
...
fi_crit = 30; % critical state friction angle [deg] - Barari: phi_cs=30
...
penetration_depths = [6.5, 5.5, 4.5, 3.5];  % penetration depths to analyze (m)
...
dd_steps = 0.5*pi/180:0.5*pi/180:3*pi/180;  % 0.25 degree steps up to 4 degrees
...
fprintf('\n\n############################################################\n');
fprintf('######  PENETRATION DEPTH: %.1f m (%d/%d)  ######\n', penetration_depths(i_pen), i_pen, num_pen);
fprintf('############################################################\n');
...
% Map computed sector (0 to <90 deg) to full 360 deg
...
for i_pen = 1:num_pen
    ...
    polarplot(tilt_dirs_rad_closed, M_closed/1000, line_styles{i_pen}, 'LineWidth', 2.5, ...
        'Color', colors_pen(i_pen,:), 'MarkerSize', 6, 'MarkerFaceColor', colors_pen(i_pen,:));
    hold on;
    legend_labels1{i_pen} = sprintf('L_p = %d m (L/D = %.2f)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b);
end
title(sprintf('Polar Moment at %.1f° Rotation (Four Pod)', dd_steps(end)*180/pi), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
legend(legend_labels1, 'fontsize', 14, 'interpreter', 'tex', 'location', 'bestoutside');
...
for i_pen = 1:num_pen
    ...
    polarplot(tilt_dirs_rad_closed, F_closed/1000, line_styles{i_pen}, 'LineWidth', 2.5, ...
        'Color', colors_pen(i_pen,:), 'MarkerSize', 6, 'MarkerFaceColor', colors_pen(i_pen,:));
    hold on;
    legend_labels2{i_pen} = sprintf('L_p = %d m (L/D = %.2f)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b);
end
title(sprintf('Polar Horizontal Force at %.1f° Rotation (Four Pod)', dd_steps(end)*180/pi), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
legend(legend_labels2, 'fontsize', 14, 'interpreter', 'tex', 'location', 'bestoutside');
set(gca, 'fontweight', 'bold', 'fontsize', 16);
ax = gca;
ax.ThetaDir = 'counterclockwise';
ax.ThetaZeroLocation = 'right';
...
for i_pen = 1:num_pen
    ...
    title(sprintf('L_p = %d m', penetration_depths(i_pen)), 'FontSize', 14, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
    set(gca, 'fontweight', 'bold', 'fontsize', 12);
    ax = gca;
    ax.ThetaDir = 'counterclockwise';
    ax.ThetaZeroLocation = 'right';
end
sgtitle('Polar Moment-Rotation Diagram (Four Pod)', 'FontSize', 18, 'FontWeight', 'bold', 'FontName', 'Times New Roman');

%% Figure 4: Moment vs Rotation (Cartesian) - all depths on one plot, first computed direction
figure(4);
legend_labels4 = {};
for i_pen = 1:num_pen
    plot(dd_steps*180/pi, squeeze(M_polar_compute(1, 1:num_steps, i_pen))/1000, line_styles{i_pen}, 'LineWidth', 2.5, ...
...
title(sprintf('Moment vs Rotation at %d° Tilt Direction (Four Pod)', tilt_directions_compute(1)), 'FontSize', 20, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
legend(legend_labels4, 'fontsize', 14, 'interpreter', 'tex', 'location', 'bestoutside');
...
for i_pen = 1:num_pen
    subplot(2, 2, i_pen);
    for i_dir = 1:num_dirs_compute
        plot(dd_steps*180/pi, squeeze(M_polar_compute(i_dir, 1:num_steps, i_pen))/1000, '-o', 'LineWidth', 2, ...
            'Color', colors_dirs(i_dir,:), 'MarkerSize', 4);
        hold on;
    end
    grid on;
    set(gca,'fontweight','bold','fontsize',12);
    xlabel('Rotation (deg)', 'FontSize', 14, 'FontWeight', 'bold','FontName','Times New Roman');
    ylabel('Moment (MNm)', 'FontSize', 14, 'FontWeight', 'bold','FontName','Times New Roman');
    title(sprintf('L_p = %d m (L/D = %.2f)', penetration_depths(i_pen), penetration_depths(i_pen)/do_b), 'FontSize', 14, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
end
sgtitle(sprintf('Moment vs Rotation (%d°-%d° sector, Four Pod)', tilt_directions_compute(1), tilt_directions_compute(end)), 'FontSize', 18, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
% Add shared legend
legend_labels5 = {};
for i_dir = 1:num_dirs_compute
    legend_labels5{i_dir} = sprintf('Dir = %d°', tilt_directions_compute(i_dir));
end
